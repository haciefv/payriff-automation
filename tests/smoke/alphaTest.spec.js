...
test("smoke: Regular 3DS payment", async ({ page }) => {
    ...
    await payPage.waitForURL(/acs\d?\.3dsecure\.az\/way4acs\/.*challenge/i, { timeout: 60000 });
    ...
    // OTP input
    const otp = payPage.locator('input#psw_id[name="PASSWORD"]');
    await otp.waitFor({ state: "visible", timeout: 60000 });

    // OTP-ni serverdən gözlə
    let otpCode;
    ...
    await otp.click();
    await otp.fill("");
    await otp.type(otpCode, { delay: 50 });
